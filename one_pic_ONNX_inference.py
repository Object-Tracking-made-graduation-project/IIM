...
def main():
    ...
    ort_sess = ort.InferenceSession('iim_net_try_1.onnx')
    ...
    jojo = predict_people_in_frame(img, ort_sess)
...
def predict_people_in_frame(img, net): # file_list -> file_path
    ...
    with torch.no_grad():
        ...
        if h * w < slice_h * 2 * slice_w * 2 and h % 16 == 0 and w % 16 == 0:
            [pred_threshold, pred_map, __] = [i.cpu() for i in net.run(img, mask_gt=None, mode='val')]
        else:
            if h % 16 != 0:
            ...
            for i in range(0, nz, period):
                #[crop_threshold, crop_pred, __] = [i.cpu() for i in net.run(crop_imgs[i:min(nz, i+period)],mask_gt = None, mode='val')]


                # тут пыытаюсь просто заинференсить сетку, но она падает 
                net.run(None, crop_imgs[i:min(nz, i+period)])
                #jojo = net.run([],crop_imgs[i:min(nz, i+period)])
                

               # [crop_threshold, crop_pred, __] = [i.cpu() for i in net.run([], crop_imgs[i:min(nz, i+period)])]
